...
object LeonardoApiClient {
  ...
  implicit def getRuntimeDoneCheckable[A]: DoneCheckable[GetRuntimeResponseCopy] =
    (op: GetRuntimeResponseCopy) => op.status == RuntimeStatus.Running
  ...
  def createRuntimeWithWait(googleProject: GoogleProject,
    ...
    for {
      ...
      res <- timer.sleep(80 seconds) >> streamFUntilDone(ioa, 30, 5 seconds).compile.lastOrError
      _ <- if (res.isDone) IO.unit
      else IO.raiseError(new TimeoutException(s"create runtime ${googleProject.value}/${runtimeName.asString}"))
    } yield ()
