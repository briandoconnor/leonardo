...
//      _ <- authProvider.notifyKubernetesClusterCreated(samResourceId)
      ...
      diskOpt <- req.diskConfig.traverse(diskReq =>
        LeoKubernetesServiceInterp.processDiskConfigRequest(
          diskReq,
          googleProject,
          false,
          userInfo,
      ...
      saveApp <- F.fromEither(
        getSaveableApp(googleProject, appName, userInfo, samResourceId, req, diskOpt, nodepool.id, ctx)
      )
...
//
  ...
  private[service] def getSaveableApp(googleProject: GoogleProject,
    ...
    val allLabels = DefaultKubernetesLabels(googleProject,
                                            appName,
                                            userInfo.userEmail,
    ...
    (req, isDataproc) match {
      case (DiskConfigRequest.Reference(name), false) =>
        for {
          ...
          _ <- if (attachedToRuntime || attachedToGKEApp)
            F.raiseError[Unit](DiskAlreadyAttachedException(googleProject, name, ctx.traceId))
          else F.unit
...
case class AppAlreadyExistsException(googleProject: GoogleProject,
                                     ...
                                     traceId: TraceId)
    extends LeoException(
      s"Kubernetes Cluster ${googleProject.value}/${appName.value} already exists in ${status.toString} status. Trace ID: ${traceId.asString}",
...
case class AppCannotBeDeletedException(googleProject: GoogleProject,
                                       ...
                                       traceId: TraceId)
    extends LeoException(
      s"Runtime ${googleProject.value}/${appName.value} cannot be deleted in ${status} status. Trace ID: ${traceId.asString}",
      StatusCodes.Conflict
    )

case class AppRequiresDiskException(googleProject: GoogleProject, appName: AppName, appType: AppType, traceId: TraceId)
    extends LeoException(
      s"Runtime ${googleProject.value}/${appName.value} cannot be created because the request does not contain a valid disk. Apps of type ${appType} require a disk. Trace ID: ${traceId.asString}",
