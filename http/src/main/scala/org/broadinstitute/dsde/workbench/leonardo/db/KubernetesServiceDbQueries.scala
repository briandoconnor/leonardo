...
object KubernetesServiceDbQueries {
  ...
  def saveOrGetForApp(saveKubernetesCluster: SaveKubernetesCluster)(implicit ec: ExecutionContext): DBIO[SaveClusterResult] =
    for {
      clusterOpt <- kubernetesClusterQuery.getMinimalActiveClusterByName(saveKubernetesCluster.googleProject)

      failure = DBIO.failed(KubernetesAppCreationException(s"You cannot create an app while a cluster is in ${KubernetesClusterStatus.Precreating} or ${KubernetesClusterStatus.Provisioning}"))
  ...
  def getActiveFullAppByName(googleProject: GoogleProject,
                             appName: AppName,
                             labelFilter: LabelMap = Map())(implicit ec: ExecutionContext): DBIO[Option[GetAppResult]] =
    joinFullAppAndUnmarshal(listByProject(Some(googleProject)),
      nodepoolQuery.filter(_.destroyedDate === dummyDate),
      appQuery.findActiveByNameQuery(appName),
      labelFilter)
      .map(clusters =>
        clusters.headOption.fold[Option[GetAppResult]](
          None
        )(cluster =>
          Some(GetAppResult(
            ...
            cluster, cluster.nodepools.head, cluster.nodepools.head.apps.head
          ))
        )
      )
  ...
  def getFullClusterById(id: KubernetesClusterLeoId)(implicit ec: ExecutionContext): DBIO[Option[KubernetesCluster]] =
    joinFullAppAndUnmarshal(kubernetesClusterQuery.findByIdQuery(id),
      nodepoolQuery,
      appQuery
    )
      .map(_.headOption)
  ...
  private[db] def joinFullApp(baseClusterQuery: Query[KubernetesClusterTable, KubernetesClusterRecord, Seq],
    ...
    val query = for {
      (((((((((app, nodepool), cluster), label), clusterNamespaces), appNamespace), services), ports), disks), diskLabels) <- baseAppQuery join
  ...
  private[db] def aggregateJoinedApp(records: Seq[(KubernetesClusterRecord, NodepoolRecord, AppRecord, Option[LabelRecord], Option[NamespaceRecord], NamespaceRecord, Option[ServiceRecord], Option[PortRecord], Option[PersistentDiskRecord], Option[LabelRecord])]
                                    ): Seq[KubernetesCluster] = {

    val map = records.toList.foldMap {
      case (clusterRecord, nodepoolRecord, appRecord, labelRecordOpt, clusterNamespaceRecordOpt, appNamespaceRecord, serviceRecord, portRecord, diskRecordOpt, diskLabelOpt) =>
        val serviceMap = serviceRecord.map(s =>
          Map(s -> Chain.fromSeq(portRecord.toList))
        ).getOrElse(Map.empty)
        ...
        val diskMap = diskRecordOpt.fold[Map[PersistentDiskRecord, Map[String, Chain[String]]]](Map())(disk => Map(disk -> diskLabelMap))
  ...
  private def unmarshalNodepoolMap(nodepools: Map[NodepoolRecord,
    Map[AppRecord,
      (Map[ServiceRecord,Chain[PortRecord]], Chain[NamespaceRecord], Map[String, Chain[String]], Map[PersistentDiskRecord, Map[String, Chain[String]]])
    ]
  ]): List[Nodepool] =
    nodepools.map { case (nodepoolRec, appMap) =>
      unmarshalNodepool(nodepoolRec, unmarshalAppMap(appMap))
    }.toSet.toList

  private def unmarshalAppMap(apps: Map[AppRecord,
    (Map[ServiceRecord,Chain[PortRecord]], Chain[NamespaceRecord], Map[String, Chain[String]], Map[PersistentDiskRecord, Map[String, Chain[String]]])
  ]): List[App] =
    apps.map { case (appRec, (serviceMap, namespaces, labelMap, diskMap)) =>
  ...
  private def unmarshalServiceMap(services: Map[ServiceRecord,Chain[PortRecord]]): List[KubernetesService] =
    services.map { case (serviceRec, portRecs) =>
