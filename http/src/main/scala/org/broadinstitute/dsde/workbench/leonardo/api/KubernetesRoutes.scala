...
import LeoRoutes.validateKubernetesName
...
import org.broadinstitute.dsde.workbench.leonardo.http.api.DiskConfigRequest.{Create, Reference}
import org.broadinstitute.dsde.workbench.leonardo.service.LeoKubernetesService

class KubernetesRoutes(kubernetesService: LeoKubernetesService[IO], userInfoDirectives: UserInfoDirectives)(
...
object KubernetesRoutes {
  ...
  implicit val diskConfigRequestEncoder: Encoder[DiskConfigRequest] = Encoder.instance { req =>
    req match {
      case create: DiskConfigRequest.Create => createEncoder(create)
