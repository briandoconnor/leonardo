...
class LeoKubernetesServiceInterpSpec extends FlatSpec with LeonardoTestSuite with TestComponent {
  ...
  it should "create an app and a new disk" in isolatedDbTest {
    ...
    val createDiskConfig = DiskConfigRequest.Create(diskName, None, None, None, Map.empty)
  ...
  it should "create an app with an existing disk" in isolatedDbTest {
    ...
    val appName = AppName("app1")
    val createDiskConfig = DiskConfigRequest.Reference(disk.name)
    val appReq = createAppRequest.copy(diskConfig = Some(createDiskConfig))
  ...
  it should "error on creation if a disk ref does not exist" in isolatedDbTest {
    val appName = AppName("app1")
    val createDiskConfig = DiskConfigRequest.Reference(diskName)
    val appReq = createAppRequest.copy(diskConfig = Some(createDiskConfig))

    val caught = the[DiskNotFoundException] thrownBy {
      kubeServiceInterp.createApp(userInfo, project, appName, appReq).unsafeRunSync()
    }

    caught.getMessage should include("not found")
  }
  ...
  it should "error on creation if a disk with that name exists" in isolatedDbTest {
    val disk = makePersistentDisk(DiskId(1)).copy(googleProject = project).save().unsafeRunSync()
    val appName = AppName("app1")
    val createDiskConfig = DiskConfigRequest.Create(disk.name, None, None, None, Map.empty)
    val appReq = createAppRequest.copy(diskConfig = Some(createDiskConfig))
  ...
  it should "list apps" in isolatedDbTest {
    ...
    val createDiskConfig1 = DiskConfigRequest.Create(diskName, None, None, None, Map.empty)
    val appReq1 = createAppRequest.copy(diskConfig = Some(createDiskConfig1))
    val diskName2 = DiskName("newDiskName")
    val createDiskConfig2 = DiskConfigRequest.Create(diskName2, None, None, None, Map.empty)
