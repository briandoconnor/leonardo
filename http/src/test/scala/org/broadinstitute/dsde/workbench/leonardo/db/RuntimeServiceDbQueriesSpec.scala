...
class RuntimeServiceDbQueriesSpec extends FlatSpecLike with TestComponent with GcsPathUtils {
  ...
  it should "list runtimes" in isolatedDbTest {
    val res = for {
      ...
      d1RuntimeConfig = RuntimeConfig.GceWithPdConfig(defaultMachineType, d1.id)
      c1 <- IO(
      ...
      d2RuntimeConfig = RuntimeConfig.GceWithPdConfig(defaultMachineType, d2.id)
      c2 <- IO(
  ...
  it should "list runtimes by labels" in isolatedDbTest {
    val res = for {
      start <- testTimer.clock.monotonic(TimeUnit.MILLISECONDS)
      d1 <- makePersistentDisk(DiskId(1)).save()
      c1RuntimeConfig = RuntimeConfig.GceWithPdConfig(defaultMachineType, d1.id)
      c1 <- IO(makeCluster(1).saveWithRuntimeConfig(c1RuntimeConfig))
      d2 <- makePersistentDisk(DiskId(2)).save()
      c2RuntimeConfig = RuntimeConfig.GceWithPdConfig(defaultMachineType, d2.id)
  ...
  it should "list runtimes including deleted" in isolatedDbTest {
    val res = for {
      ...
      c3RuntimeConfig = RuntimeConfig.GceWithPdConfig(defaultMachineType, d3.id)
      c3 <- IO(
        makeCluster(3).saveWithRuntimeConfig(RuntimeConfig.GceWithPdConfig(defaultMachineType, d3.id))
      )
  ...
  it should "get a runtime" in isolatedDbTest {
    val res = for {
      ...
      c1RuntimeConfig = RuntimeConfig.GceWithPdConfig(defaultMachineType, disk.id)
      c1 <- IO(makeCluster(1).saveWithRuntimeConfig(c1RuntimeConfig))
