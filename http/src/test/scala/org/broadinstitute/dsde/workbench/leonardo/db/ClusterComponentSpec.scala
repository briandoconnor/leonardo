...
class ClusterComponentSpec extends FlatSpecLike with TestComponent with GcsPathUtils with ScalaFutures {
  ...
  it should "persist persistentDiskId foreign key" in isolatedDbTest {
    val res = for {
      ...
      savedRuntime <- IO(
        makeCluster(1).saveWithRuntimeConfig(RuntimeConfig.GceWithPdConfig(defaultMachineType, savedDisk.id))
      )
      ...
      error <- IO(makeCluster(2).saveWithRuntimeConfig(RuntimeConfig.GceWithPdConfig(defaultMachineType, DiskId(-1)))).attempt
    } yield {
